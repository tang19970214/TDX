<template>
  <div class="commonPage" v-if="!!list">
    ...
    <div class="commonPage__commonCard commonPage__firstCard" :class="{'flex-row': !checkJob()}">
      <div :class="{'commonPage__firstCard--left': checkJob(), 'no-chart': !checkJob()}">
        <div class="score">
      ...
      <div class="commonPage__firstCard--right" v-if="checkJob()">
        <ChartAllFinishBar :labels="['企業平均整體分數', '產業平均', '產業高標']" :series="[{data: [getItem(0).myAvg, getItem(0).orgAvg, getItem(0).orgHight]}]" :changeColor="true" />
      </div>
    </div>

    <div class="commonPage__title">
      <strong>六大次構面細部分析</strong>
    </div>
    ...
    <div class="commonPage__commonCard commonPage__secondCard">
      ...
      <section>
        <div class="commonPage__secondCard--main">
          ...
          <div class="right" v-if="checkJob()">
            <ChartAllFinishBar height="250" :labels="[getItem(1).subName, getItem(2).subName]" :series="[{name: '企業平均整體分數', data: [getItem(1).myAvg, getItem(2).myAvg]},{name: '產業平均', data: [getItem(1).orgAvg, getItem(2).orgAvg]}, {name: '產業高標', data: [getItem(1).orgHight, getItem(2).orgHight]}]" />
          </div>
        </div>
      ...
      <section>
        <div class="commonPage__secondCard--main">
          ...
          <div class="right" v-if="checkJob()">
            <ChartAllFinishBar height="250" :labels="[getItem(3).subName, getItem(4).subName]" :series="[{name: '企業平均整體分數', data: [getItem(3).myAvg, getItem(4).myAvg]},{name: '產業平均', data: [getItem(3).orgAvg, getItem(4).orgAvg]}, {name: '產業高標', data: [getItem(3).orgHight, getItem(4).orgHight]}]" />
          </div>
        </div>
        ...
        <div class="commonPage__secondCard--content">
          ...
          <div class="result">
            ...
            <ul>
              <li v-for="(item, idx) in resultSuggest[4].score[checkScore(getItem(4))].content" :key="idx">
                {{item.text}}
              </li>
            </ul>
          </div>
      ...
      <section>
        <div class="commonPage__secondCard--main">
          ...
          <div class="right" v-if="checkJob()">
            <ChartAllFinishBar height="250" :labels="[getItem(5).subName, getItem(6).subName]" :series="[{name: '企業平均整體分數', data: [getItem(5).myAvg, getItem(6).myAvg]},{name: '產業平均', data: [getItem(5).orgAvg, getItem(6).orgAvg]}, {name: '產業高標', data: [getItem(5).orgHight, getItem(6).orgHight]}]" />
          </div>
        </div>
    ...
    <div class="commonPage__commonCard commonPage__thirdCard">
      ...
      <div class="commonPage__thirdCard--body">
        <label>若有相關問題，請聯繫TDX團隊 劉先生 2586-5000 ext.298 王先生2586-5000 ext.216</label>

        <p>數位轉型相關補助案：</p>

        <ul>
          <li>雲市集工業館數位點數補助計畫</li>
          <li>中小製造業接班傳承數位轉型主題式研發計畫</li>
          <li>消費數據驅動精準研製造(C2M)補助主題式研計畫</li>
          <li>金屬製品數位轉型加值主題式研發計畫</li>
          <li>數位轉型服務團(快易通)諮詢</li>
        </ul>
      </div>
    </div>
  </div>
